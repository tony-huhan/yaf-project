$('document').ready(function() {
	...
	var listSellerViewModel = function() {
        ...
        self.searchField = ko.observableArray(['nickname', 'email', 'mobile']);
        // 搜索
		...
		// 校验错误信息
		self.setErrors = ko.validation.group([self.nickname, self.email, self.password, self.mobile, self.note]);
		// 校验是否通过
		...
		self.setClick = function() {
			self.opItem(this);
			self.setNickname(this.nickname);
		...
		self.set = function() {
			...
			if (self.setNickname.hasChanges()) {
				modelData.nickname = self.setNickname();	
			}				
            ...
            modelData.sellerid = self.opItem().sellerid;
		...
		$.ajax({
			...
			switch (res.code) {
				case 1:
					...
					var i = 0;
					$.each(res.sellerList, function(index, data) {
